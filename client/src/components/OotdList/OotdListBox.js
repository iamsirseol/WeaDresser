...
function OotdListBox(){
    ...
    useEffect(() => {// 현재 이미지를 링크로 불러오는데 아무래도 속도가 느려 images 변수값의 offestWidth 가 undefind 가 뜰때가 있는데 그걸 방지하기 위해 넣어놨다.
        ...
        window.addEventListener('resize', function(){
            if(window.matchMedia("(max-width: 400px)").matches && window.location.pathname === "/ootd-list"){
                ootdListGrid([0,0])
            }else if(window.matchMedia("(max-width: 800px)").matches && window.location.pathname === "/ootd-list"){
                ootdListGrid([0,0,0])
            }else if(window.matchMedia("(max-width: 1200px)").matches && window.location.pathname === "/ootd-list"){
                ootdListGrid([0,0,0,0])
            }else if(window.location.pathname === "/ootd-list"){
                ootdListGrid([0,0,0,0,0])
            }
        })
